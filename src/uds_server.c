/**
 * @file uds_server.c
 * @brief UDS Core Server Treiber für Zephyr RTOS v4.4.0
 */
...
static void uds_send_nrc(uint8_t sid, uint8_t nrc)
{
    if (current_msg_addressing == UDS_ADDR_FUNCTIONAL) {
        if (nrc == UDS_NRC_SERVICE_NOT_SUPPORTED || nrc == UDS_NRC_SUB_FUNCTION_NOT_SUPPORTED || nrc == UDS_NRC_SUB_FUNCTION_NOT_SUPPORTED_IN_ACTIVE_SESS) return;
    }
