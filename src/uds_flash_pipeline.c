                                 ...
                                 void (*nrc_cb)(uint8_t, uint8_t))
{
    ...
    LOG_INF("Request Download akzeptiert. Addr: 0x%08X, Size: %u Bytes", memory_address, memory_size);
                              ...
                              void (*nrc_cb)(uint8_t, uint8_t))
{
    ...
    if (block_counter != expected_block_counter) {
        LOG_ERR("Falscher Block Counter! Erwartet: %u, Erhalten: %u", expected_block_counter, block_counter);
    ...
    LOG_INF("Block %u empfangen (%zu Bytes). Gesamt: %u/%u", block_counter, payload_len, bytes_received, memory_size);
                                      ...
                                      void (*nrc_cb)(uint8_t, uint8_t))
{
    ...
    LOG_INF("Transfer beendet. Insgesamt %u Bytes geflasht.", bytes_received);
